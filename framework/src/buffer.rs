...
impl VertexArray
{
    pub fn new() -> Self
    {
        ...
        VertexArray{ id: id, refs: HashMap::new() }
    }
...
impl<T> Buffer<T>
{
    ...
    pub fn write(&self, offset: usize, data: &[T])
    {
        ...
        unsafe{ gl::NamedBufferSubData(self.id, byte_off as GLintptr, mem::size_of_val(data) as GLsizeiptr, data.as_ptr() as *const _) };
    }
    ...
    pub fn copy_into(&self, dest: &Buffer<T>, src_idx: usize, dst_idx: usize, len: usize)
    {
        ...
        unsafe{ gl::CopyNamedBufferSubData(self.id, dest.id, src_offset as GLsizeiptr, dst_offset as GLsizeiptr, byte_len as GLsizeiptr) };
    }
...
impl<T> BufferAny for Buffer<T>
{
    fn empty() -> Self
    {
        unsafe
        {
            ...
            Buffer{ id: id, size: 0, usage: 0, _t: PhantomData }
        }
    }
